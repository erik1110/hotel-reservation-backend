...
export class OrderService {
  ...
  async createOrder(req: Request, createOrderDto: CreateOrderDto) {
    ...
    const existingOrders = await this.orderModel.find({
      ...
      $or: [
        {
          checkInDate: { $lt: createOrderDto.checkOutDate },
          checkOutDate: { $gt: createOrderDto.checkInDate },
        },
        {
          checkInDate: { $lt: createOrderDto.checkInDate },
          checkOutDate: { $gt: createOrderDto.checkOutDate },
        },
      ],
    });
  ...
  async updateOrderAdmin(
    ...
    const existingOrders = await this.orderModel.find({
      ...
      $or: [
        {
          checkInDate: { $lt: updateOrderDto.checkOutDate },
          checkOutDate: { $gt: updateOrderDto.checkInDate },
        },
        {
          checkInDate: { $lt: updateOrderDto.checkInDate },
          checkOutDate: { $gt: updateOrderDto.checkOutDate },
        },
      ],
    });
    if (existingOrders.length > 0) {
